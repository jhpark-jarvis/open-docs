...
OpenDocs는 특정 공지 정보를 자동 수집하고, OCR 추출, 구조화, 문서화까지 이어가는 AI 기반 문서 플랫폼의 모노레포입니다.
...
로컬에서 백엔드 테스트만 빠르게 실행하려면 `apps/api/.venv` 가상환경을 사용합니다.

1. 가상환경 활성화:
   `.\apps\api\.venv\Scripts\Activate.ps1`
2. 테스트 실행:
   `python -m pytest apps/api/tests/test_nexon_notice_active.py`

활성화 없이 바로 실행하려면 다음 명령을 사용합니다.

`.\apps\api\.venv\Scripts\python.exe -m pytest apps/api/tests/test_nexon_notice_active.py`

가상환경이 없으면 `apps/api` 기준으로 개발 의존성을 설치합니다.
...
## DB Sync(Alembic)
- Alembic docker 반영 : `docker compose exec api alembic upgrade head`
- 현재 상태 확인 : `docker compose exec api alembic current`
